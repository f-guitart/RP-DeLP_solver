...
class almostValidList(dict):

    def insert_list(self,p , nd_list, vc):
        for lit in nd_list:
            self.insert(p,nd_list,vc)

    def insert(self, p , q, vc):
        if (p,q,vc) not in self.keys():
            self[(p,q)]=vc
    ...
    def depend(self,p,q,vc):
        return (p,q,vc) in self.keys()

class reasonList(dict):

    def add(self,lit,r,level):
        if lit in self.keys():
            self[lit].append((r,level))
        else:
            self[lit]=[(r,level)]

class stack(list):

    def push2(self,W,B,alpha):
        self.append((W,B,alpha))

    def push(self,W,W_r,B,B_r,alpha):
        self.append((W,W_r,B,B_r,alpha))

    def is_empty(self):
        return len(self)==0

    def is_not_in(self,W1,B1,alpha1):
        print "========", W1,B1,alpha1
        for W,W_r,B,B_r,alpha in self:
            print "********",W,B,alpha,W==W1,B==B1,alpha==alpha1
            if set(W)==set(W1) and set(B)==set(B1) and alpha==alpha1:
                return False
        return True
...
class inverseList(dict):
    ...
    def insert(self,rule,c,level):
        for lit in rule.getBody():
            if lit in self.keys():
                self[lit].append((c,level))
            else:
                self[lit]=[(c,level)]

    def insert2(self,rule,c):
        for lit in rule.getBody():
            if lit in self.keys():
                self[lit].append(c)
            else:
...
class inverseRulesList(list):
    def __init__(self,program):
        for level in range(0,len(program)):
            act_lvl={}
            for rule in program[level]:
                if rule.getBody() is not None:
                    for literal in rule.getBody():
                        if literal in act_lvl.keys():
                            act_lvl[literal].append((rule,level))
                        else:
                            act_lvl[literal]=[(rule,level)]
            self.append(act_lvl)
...
class rulesList(object):
    ...
    def insert(self,clause,W,B,level):
        ...
        if type(clause) is list:
            ...
            if clause.condition_v3(W,B) and clause.has_blocked(B) is False:
                ...
                c=clause[0]
                self.inv_list.insert(c,clause,level)
    ...
    def get_prevalids(self,p,W,B,level):
        ...
        prevalids=[]
        if self.level<level:
            for l in p.clauses[0:level]:
                for clause in l:
                    if clause.getBody() is not None:
                        #check rule
                        c=metaRule(clause,W,B)
                        p.rulesList.insert(c,W,B,level)
                        if is_prevalid(clause,W,B):
                            prevalids.append(clause.getHead())
                    else:
                        if is_prevalid(clause.getHead(),W,B):
                            prevalids.append(clause.getHead())
        ...
        if l in self.head_list.keys():
            for mrule in self.head_list[l]:
                if mrule[1][0] == mrule[1][1]:
                    rules.append(mrule[0])
        return rules

    def get_producing_rule2(self,p,l,W):
        rules=[]
    	for level in p.clauses:
	    for rule in level:
	       	if rule[0] == l:
                    if len(rule)>1:
                        if set(rule[1]).issubset(set(W)):
                            rules.append(rule[1])
                    else:
		        rules.append(rule[0])
        return rules
...
def is_prevalid(l,W,B):
    ...
    if isinstance(l,Clause):
        if l.getBody() is None:
        #    print "  --",l.getHead()
            return is_prevalid(l.getHead(),W,B)
...
class stats():
    ...
    def new_output(self):
        ...
        if len( self.warrant_list)>0:
            self.warrant_list.append(copy.copy(self.warrant_list[-1]))
            self.blocked_list.append(copy.copy(self.blocked_list[-1]))
        else:
            self.warrant_list.append([])
    ...
    def update(self,W,B):
        ...
        #get last output to maintain warrant stats for new output
        if len(self.warrant_list) > 0:
            last_output_w=self.warrant_list[-1]
            last_output_b=self.blocked_list[-1]

        for lit in last_output_w:
            if lit[0] not in W:
                last_output_w.remove(lit)
                
        for lit in last_output_b:
            if lit[0] not in B:
                last_output_b.remove(lit)

    def set_warranted(self,lit,rule,level):
        self.warrant_list[-1].append((lit,rule,level))

#    def set_blocked(self,lit,blit,reason,level):
#        print "==***********=> Add blocked... ",lit, blit,reason, level
#        self.blocked_list[-1].append((lit,blit,reason,level))

    def set_blocked(self,p,lit,block_reason,blit,W,vc,level):        
        prod_rules=p.getBodyByHead(level,lit)
        rul=[]
        if prod_rules:
            for rule in prod_rules:
                p.is_active_rule(rule,W,level)
                rul.append(rule)

        self.blocked_list[-1].append((lit,block_reason,blit,rul,level))            
        #c=raw_input("***********_________%s___________*****************" % rul)
        
    def add_time(self,t):
        self.times.append(t)

    def set_blocked_reason(self,blit,reason):
        #reason must be a list
        if blit in self.blocked_reasons.keys():
            self.blocked_reasons[blit].append(reason)
        else:
            self.blocked_reasons[blit]=[reason]

    def get_blocked_reason(self,blit):
        if blit in self.blocked_reasons.keys():
            return self.blocked_reasons[blit]
        else:
            return ["no reason"]

    def set_warrant_reason(self,wlit,reason):
        if wlit in self.warrant_reasons.keys():
            self.warrant_reasons[wlit].append(reason)
        else:
            self.warrant_reasons[wlit]=[reason]

    def get_warrant_reason(self,wlit):
        if wlit in self.warrant_reasons.keys():
            return self.warrant_reasons[wlit]
        else:
            return ["no reason"]

    def update_reasons(self,W,B):
        new_block_reasons={}
        for b_lit in B:
            new_block_reasons[b_lit]=self.get_blocked_reason(b_lit)
        self.blocked_reasons=new_block_reasons

        new_warrant_reasons={}
        for w_lit in B:
            new_warrant_reasons[w_lit]=self.get_warrant_reason(w_lit)
        self.warrant_reasons=new_warrant_reasons
...
class validLiteralsList(object):
    ...
    def is_cycle(self,lit):
        if self.exists(lit):
            return self.list[lit][2]

    def is_conflict(self,lit):
        if self.exists(lit):
            return self.list[lit][3]
    ...
    def next(self):
        ...
        if self.none_valid:
            ...
            if current_item<len(self.list):
                self.current_time+=1
            else:
               self.current_time+=0
        
            if self.is_valid(self.list[self.current_item]):
                return self.list[self.current_item]
            else:
                return self.next()
            

class almostValidCache(dict):
#'''It holds a list of almost valid arguments for a tuple (P,Q)'''
 
    def findKeys(self, val):
        '''
        return the key of dictionary dic when v in value
        '''
        heads=[]
        #if v is None ther is an exception, any better idea?
        try: heads.extend([k for k, v in self.iteritems() if val in v])
        except TypeError:
            pass
        return heads    

    
    def update(self, blockLit):
        '''delete items appearing v in its value'''
        for key in self.findKeys(blockLit):
            del self[key]
...
class Program(object):
    ...
    def get_strict_knowledge(self):
        ...
        if len(self.clauses)<1 or len(self.clauses[0])<1:
            #print "@@@@ No strict knowledge"
            return True
        ...
        if(result != False):
            if(result):
                for var in result:
                    ...
                    #"@@@@ Added "+var+" as warranted in STRICT KNOWLEDGE"
            return True
        else:
            return False
    ...
    def get_valid_literals(self,W,B,level):
        ...
        vc=[]
        #pre_vc=self.rulesList.get_prevalids(self,W,B,level)
        #get prevalids
        pre_vc=[]
        for l in self.clauses[0:level+1]:
            for clause in l:
                if is_prevalid(clause,W,B):
                    pre_vc.append(clause.getHead())

        #check if new literals generates conflict
        for literal in pre_vc:
            if not generates_conflict(self,literal,W) and literal not in vc:
                print "#### Checking valid literals->>>>",literal, "  do not generate conflict"
                vc.append(literal)
...
#                print "###Checking if %s is valid" % rule.head
    ...
    def get_alpha_rules(self, level, W, B):
        arules=[]
        for l in range(0,level+1):
            for clause in self.clauses[l]:
                ...
                if clause.body:
                    ...
                    if (set(clause.body).difference(set(W)) and l<level) or l>=level:
                        if not set(clause.body).intersection(set(B)):
                            if clause.head not in set((set(W)).union(set(B))) and clause.head.neg() \
                                    not in set((set(W)).union(B)):
                                arules.append(clause)
        return arules

    def is_active_rule(self,rule,W,level):
        for lit in rule:
            if lit not in W:
                return False
        return True
...
def generates_conflict(program, literal, W):
        '''Check if literal l generates conflict with strict knowledge plus
        warranted literals
        Warning: As long as negated literals are used as dual literals, we add the following implication
        for every rule head and warranted literal literal -> -(~literal)
...
def not_depend(program, W, B, Q, vc, av_list, level, solver):
    not_dependent_lit=[]
    for clause in program.clauses[level]:
        if clause[0] not in vc and clause[0] not in not_dependent_lit and not av_list.depend(clause[0],Q,vc):
            if solver=="minisat" or solver=="glucose":
                if almost_validSAT(program, W, B, Q, clause[0], vc, av_list, level, solver):
                    not_dependent_lit.append(clause[0])
                    av_list.insert(clause[0],Q,vc)
                else:
                    pass
            elif solver=="clingo":
                if almost_validASP(program, W, B, Q, clause[0], vc, av_list, level, solver):
...
def conflict(program,level,Q,validLiterals,W,D,call,solver):

    if solver=="glucose" or solver=="minisat":
        return conflictSAT(program,level,Q,validLiterals,W,D,call)
    elif solver=="clingo":
        return conflictASP(program,level,Q,validLiterals,W,D,call)

def conflictSAT(program,level,Q,validLiterals,W,D,call):
    global numEnc
    ...
    if Q.neg() in tlS1[0]:
        #print tlS1[0], "conlficte a la vista", Q
        return True,Q.neg()


    if(call==1):
        program.stats.conflict1Calls+=1
    else:
    ...
    while(i<=1):
        for clause in program.clauses[0]:
            if (clause.getBody() != None) and (clause.getHead() not in W):
    ...
    while(tlS2[i] != tlS2[i+1] or trS2[i] != trS2[i-1]):
        i=i+1
        for clause in program.clauses[0]:
            if (clause.body and clause.head not in W):
                if set(clause.body).issubset(tlS2[i]):
                    trS2[i].add((clause.head,tuple(clause.body)))
        trS2.append(set())
        tlS2.append(tlS2[i].copy())
...
#    print "Conflicts:",conflicts
    ...
    cnf=ConjunctiveNormalFormula("%s-%i" % (program.fileName,numEnc))
    cnf.comment("Query q:"+Q+", vc:"+str(validLiterals)+", wl:"+str(W)+", D:"+str(D)+"\n")
    cnf.comment("Warranted:"+str(W))
...
#                return True,conflict,solution[1].index(conflict)
                return True,act_lit
...
def conflictASP(program,level,Q,validLiterals,W,D,call):
    global numEnc
    numEnc=numEnc+1
    ...
    #print "\n==============QUERY============"
    #print "Query: Q: "+Q+" ValidLiterals: "+str(validLiterals)+" WarrantedLiterals: "+str(W)+" D: "+str(D)+"\n"
    #generate sets
    #set of predicates
    ...
    if len(conflicts2)==0: return False,"a"
    ...
    if(DEBUG):
        ...
        f.write("==============QUERY============\n")
        f.write("Query: Q: "+Q+" ValidLiterals: "+str(validLiterals)+" WarrantedLiterals: "+str(W)+" D: "+str(D)+"\n")
        f.write("==============SOLUTION============\n")
        f.write(str(sol[2])+"\n")
        if sol[0]==True:
            fsol={}
...
def almost_validSAT(program, W, B, q, p, vc, av_list, level, method):
    global numEnc
    enctim1=time.time()
    ...
    if p in W or p in B or Literal(p).neg() in B:
    ...
    strict_rules=[rule for rule in program.getStrictRules() if rule[0] not in B]
    alpha_rules=program.get_alpha_rules(level,W,B)
    i=0
    while(i<=1):
    ...
    for lit in literals[0]:
        argument=[]
        if (lit not in W) and (literals[t][lit] in solution):
            argument.append(lit)
    ...
    if DEBUG:
        ...
        f.write("Warranted: "+str(W))
        f.write("Blocked: "+str(B))
        f.write("=================PROGRAM================\n")
        f.write(str(program))
        f.write("=================SOLUTION================\n")
...
def almost_validASP(program, W, B, q, p, vc, av_list, level, method):
#(program,q,p,vc,level)
    global numEnc
    numEnc=numEnc+1
...
#    else:
        #print "!!!Not need to check!!!!"
#        if(argument==None):
            #print "Almost valid cache"
#            return False
#        else:
            #print "Almost valid cache"
#            return True
    ...
    asp.addComment("Warranted: "+str(W))
